...
Users_vc = pd.read_csv('./data/train_c.csv', header = 0, encoding = 'utf-8')
...
class AlbertClassifier(torch.nn.Module):
    def __init__(self, bert_model, bert_config, num_class):
        ...
        self.dropout = torch.nn.Dropout(p = 0.1, inplace = False)           #加入dropout
...
optimizer = torch.optim.AdamW(params = albertBertClassifier.parameters(), lr = learn_rate)      #AdamW正则化优化器

total_steps = (num_data // batch_size) * epoch if num_data % batch_size == 0 else (num_data // batch_size + 1) * epoch
scheduler = get_linear_schedule_with_warmup(optimizer= optimizer, num_warmup_steps= warmup_ratio * total_steps, num_training_steps= total_steps) #warmup控制器
...
if __name__ == "__main__":
    ...
    for epoch in tqdm(range(epoch)):
        ...
        if test_accu/len(test_dataset) > test_accu_max:
            ...
            count_smaller += 1    #如果连续50次未对最大值进行刷新则判断过拟合
            if count_smaller >= 50:
